# Budget (token_rate_limiter plugin) Specification

## Purpose

The `token_rate_limiter` plugin governs spend by bounding consumption per
consumer or per gateway. It unifies cumulative token budgets, dollar budgets,
and a stateless per-model cost cap into a single plugin that extends the
existing `token_rate_limiter` in place. The slug stays `token_rate_limiter`.
All new fields are optional; absent fields preserve the pre-change behavior.
Scope is always derived from `Policy.Global` (never config). Cohere is out of
scope.
