...
- [ ] 2.1 Create `evaluate.go`: `aggregated{scores,flagged}`, `aggregate([]moderationResult)` (max-per-category + flagged OR across all `results[]`), `evaluate(cfg, agg) []violation` (allow-list when non-empty else union of response categories; threshold-crossed or `block_on_flagged`+flagged; one per category; sorted/deterministic).
- [ ] 2.2 Create `reject.go`: type/message consts (`content_flagged`, `moderation_unavailable`, default block message), `blockBody([]violation)`, `blockError(message, violations)` → 403 `*appplugins.PluginError`, `unavailableError()` → 502 fixed generic body (no OpenAI detail).
