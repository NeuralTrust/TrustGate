...
- [x] 9.3 `plugins/regexreplace/replace.go`, `bedrockguardrail/anonymize.go`, `trustguard/rewrite.go`, `promptcompression/rewrite.go`: baseline clone + graft on the change path. `googlemodelarmor` does not exist on this base (origin/main): nothing to wire.
