...
- [x] 5.4 `cache_intent.go`: `isGPT56OrLater` (one-digit major, vendor prefix, suffixes); `cacheProfileFor(target, model)`. Responses GPT-5.6+: system+messages, max 4 (3 unless `mode=explicit`). openai/azure/Responses: key always, `prompt_cache_options` only on GPT-5.6+, `prompt_cache_retention` only before GPT-5.6 (OpenAI/Azure docs: 400 on options before 5.6, retention deprecated on 5.6). **xAI Chat: nothing** (xAI Chat caches by the `x-grok-conv-id` header, not a body key) — deviation from D3. Review fixes: the profile is gated on the target provider (`AdaptRequestForProvider`), so Cerebras and openai_compatible get nothing; Azure gets `prompt_cache_key` only (deployment names defeat the GPT-5.6 gate); Responses drops assistant and image markers before the cap; `mode: "explicit"` is removed when no breakpoint is left; image markers stay on the image; `isGPT56OrLater` takes `ft:` names and two-digit majors.
...
- [x] 9.1 `adapter/graft.go` (new) `GraftChangedFields`, `CanonicalRequest.Clone`; `adapter/tools.go` (new) `FilterTools` and the tools-array graft; `adapter/rawjson.go` (new) byte-offset JSON scanner and patcher; tests.
- [x] 9.2 `plugins/{toolinjection,toolallowlist,pertoolratelimit}/plugin.go`: shared graft (three copies of `graftChangedFields` removed); allowlist and pertoolratelimit use `FilterTools`; toolinjection already copied `Cache` on in-place replace.
- [x] 9.3 `plugins/promptcompression/rewrite.go`: baseline clone + graft on the change path. Redaction plugins (`regexreplace/replace.go`, `bedrockguardrail/anonymize.go`, `trustguard/rewrite.go`) re-encode in full once they change text (markers kept by the S2/S4 encoders, unmodelled fields dropped so no copy of a masked value survives); an unchanged mask forwards the original bytes. `googlemodelarmor` does not exist on this base (origin/main): nothing to wire.
- [x] 9.4 Plugin tests: each plugin-cache-preservation scenario (regexreplace on Anthropic, anonymize on Responses and Bedrock, trustguard mask and response usage, allowlist removal and marker move, injection, pertoolratelimit strip); no-op byte-identical; Codex Responses body keeps reasoning/include/store/parallel_tool_calls/tool_choice/strict.
- [x] 9.5 `docs/prompt-caching.md` (new): usage accounting, intent profile table, Bedrock capability table, passthrough/re-encode rules, lossy cases, plugin behaviour and prefix-stability notes.
- [ ] 9.6 V1 (adapter, `pkg/infra/plugins/...`); V2; V3; V4 **Anthropic, OpenAI, openai_responses** (plugin cases) plus Bedrock for bedrockguardrail; V5. *(V1 done: gofmt, vet, golangci-lint 0 issues on adapter + plugins, race tests providers+plugins+proxy, `go test ./pkg/...`, reviewer harnesses and 60 s fuzz runs, after the second review round; V2-V5 pending.)*
