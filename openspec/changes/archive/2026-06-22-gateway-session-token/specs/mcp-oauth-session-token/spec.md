# Delta for MCP OAuth Session Token

New capability: the gateway mints, validates locally, and refreshes its own MCP
session token, decoupled from the upstream IdP token format. Behavior is opt-in
per OAuth2 auth; OFF preserves today's IdP pass-through. RFC 2119 keywords apply.
