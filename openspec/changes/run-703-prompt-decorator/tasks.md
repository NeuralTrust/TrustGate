...
- [ ] 3.1 Create `document.go` to copy current `Body` into private RawMessages, preserve unknown/rich fields, and marshal separately allocated output without input edits.
...
- [ ] 4.1 Extend `document.go` for copied top-level system strings/text arrays and user/assistant messages without flattening blocks.
- [ ] 4.2 Test anchors, strategies, ordering, blank systems, fidelity, and alias safety. Verify: `go test -race ./pkg/infra/plugins/promptdecorator`.
