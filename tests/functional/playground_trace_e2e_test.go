...
// putPlaygroundTrace pushes a trace into the admin plane the way a hybrid data
// plane does, returning the response status.
func putPlaygroundTrace(t *testing.T, traceID, body string) int {
	t.Helper()
	req, err := http.NewRequest(http.MethodPut, AdminURL+"/v1/playground/traces/"+traceID, strings.NewReader(body))
	require.NoError(t, err)
	req.Header.Set("Authorization", "Bearer "+AdminToken)
	req.Header.Set("Content-Type", "application/json")
...
// TestPlaygroundTraceIngestE2E covers the hybrid push path: a trace PUT into
// the control-plane store must be retrievable by trace id, exactly like one
// captured by the local proxy.
func TestPlaygroundTraceIngestE2E(t *testing.T) {
	defer Track(t, "PlaygroundTrace")()

	t.Run("pushed trace is retrievable by trace id", func(t *testing.T) {
		traceID := uuid.NewString()
		status := putPlaygroundTrace(t, traceID,
			`{"trace_id":"`+traceID+`","gateway_id":"gw-hybrid","status":{"code":200}}`)
		require.Equal(t, http.StatusNoContent, status)

		getStatus, raw := getPlaygroundTrace(t, traceID)
		require.Equal(t, http.StatusOK, getStatus, "body: %s", raw)
		...
		assert.Equal(t, traceID, evt.TraceID)
		assert.Equal(t, "gw-hybrid", evt.GatewayID)
	})

	t.Run("mismatched trace id is rejected", func(t *testing.T) {
		status := putPlaygroundTrace(t, uuid.NewString(), `{"trace_id":"another-trace"}`)
		assert.Equal(t, http.StatusBadRequest, status)
	})

	t.Run("unauthenticated push is rejected", func(t *testing.T) {
		traceID := uuid.NewString()
		req, err := http.NewRequest(http.MethodPut, AdminURL+"/v1/playground/traces/"+traceID,
			strings.NewReader(`{"trace_id":"`+traceID+`"}`))
		require.NoError(t, err)
		req.Header.Set("Content-Type", "application/json")
