...
// Track is a thin wrapper over RunTest that captures the start time itself, so
// individual test files need no time import. Usage: defer Track(t, "Suite")()
func Track(t *testing.T, suite string) func() {
	return RunTest(t, suite, time.Now())
}
