...
var pluginCatalogMeta = map[string]catalogMeta{
	...
	"per_tool_rate_limiter": {
		...
		group:       groupTrafficControl,
		description: "Count and enforce limits per real tool execution, counted on pre_request from the tool results (role \"tool\") the client returns on the next request. The limit applies gateway-wide when the policy is global, otherwise per consumer.",
		schema: SettingsSchema{
			Fields: []Field{
				{
					Key:         "rules",
