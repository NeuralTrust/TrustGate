...
type CreateInput struct {
	...
	// Entitlements overrides the default tier; internal-only, never set from client input.
...
func (c *creator) Create(ctx context.Context, in CreateInput) (*domain.Gateway, error) {
	...
	if err := c.checkInstanceLimit(ctx, in.TenantID, g.Entitlements); err != nil {
		return nil, err
	}
...
// checkInstanceLimit enforces the tier's max-instances cap per tenant; skipped when there is no tenant.
func (c *creator) checkInstanceLimit(ctx context.Context, tenantID string, entitlements domain.Entitlements) error {
	if tenantID == "" {
		return nil
	}
	...
	count, err := c.repo.CountByTenantID(ctx, tenantID)
	if err != nil {
		return err
	}
