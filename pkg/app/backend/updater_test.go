...
func TestUpdater_Update_Success(t *testing.T) {
	...
	existing, _ := domain.NewBackend(ids.New[ids.GatewayKind](), "old", "openai", nil, "", 1, domain.NewAPIKeyAuth("sk-1"), nil)
...
func TestUpdater_Update_RejectsGatewayIDChange(t *testing.T) {
	t.Parallel()
	repo := repomocks.NewRepository(t)
	existing, _ := domain.NewBackend(ids.New[ids.GatewayKind](), "x", "openai", nil, "", 1, domain.NewAPIKeyAuth("sk-1"), nil)
	repo.EXPECT().FindByID(mock.Anything, existing.ID).Return(existing, nil).Once()
