...
// SambaNova Cloud is OpenAI wire-compatible: same chat-completions shape,
// Bearer auth, and SSE frames terminated by "data: [DONE]".
const chatCompletionsURL = "https://api.sambanova.ai/v1/chat/completions"

type client struct {
	chat *openai.ChatCompletionsClient
}

func NewSambaNovaClient() providers.Client {
	return &client{
