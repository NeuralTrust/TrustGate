...
// fireworksClientAt is a test-only wrapper that targets a custom URL so the
...
func TestCompletions_NonStreamingRoundTrip(t *testing.T) {
	const wantModel = "accounts/fireworks/models/llama-v3p3-70b-instruct"
...
func TestCompletionsStream_RoundTrip(t *testing.T) {
	...
	seq, err := newFireworksClientAt(srv.URL+"/chat/completions").CompletionsStream(
		...
		[]byte(`{"model":"accounts/fireworks/models/llama-v3p3-70b-instruct","stream":true}`),
	)
	require.NoError(t, err)
...
func TestCompletions_RateLimitRetryAfter(t *testing.T) {
	...
	_, err := newFireworksClientAt(srv.URL+"/chat/completions").Completions(
		...
		[]byte(`{"model":"accounts/fireworks/models/llama-v3p3-70b-instruct","messages":[{"role":"user","content":"hi"}]}`),
	)
	require.Error(t, err)
