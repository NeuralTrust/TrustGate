...
// perplexityClientAt is a test-only wrapper that targets a custom URL so the
...
func TestCompletions_NonStreamingRoundTrip(t *testing.T) {
	...
	resp, err := newPerplexityClientAt(srv.URL+"/chat/completions").Completions(
		context.Background(),
...
func TestCompletionsStream_RoundTrip(t *testing.T) {
	...
	seq, err := newPerplexityClientAt(srv.URL+"/chat/completions").CompletionsStream(
		context.Background(),
		&providers.Config{Credentials: providers.Credentials{ApiKey: "perplexity-key"}},
		[]byte(`{"model":"sonar-pro","stream":true}`),
	)
	require.NoError(t, err)
...
func TestCompletions_RateLimitRetryAfter(t *testing.T) {
	...
	_, err := newPerplexityClientAt(srv.URL+"/chat/completions").Completions(
		context.Background(),
		&providers.Config{Credentials: providers.Credentials{ApiKey: "key"}},
		[]byte(`{"model":"sonar-pro","messages":[{"role":"user","content":"hi"}]}`),
	)
	require.Error(t, err)
