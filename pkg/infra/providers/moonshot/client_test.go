...
// moonshotClientAt is a test-only wrapper that targets a custom URL so the
// OpenAI-compatible round trip can be exercised against an httptest server.
type moonshotClientAt struct {
	chat *openai.ChatCompletionsClient
	url  string
}

func newMoonshotClientAt(url string) providers.Client {
	return &moonshotClientAt{
		chat: openai.NewChatCompletionsClient(providers.ProviderMoonshot, nil),
		url:  url,
	}
}

func (c *moonshotClientAt) Completions(ctx context.Context, config *providers.Config, reqBody []byte) ([]byte, error) {
	return c.chat.Completions(ctx, c.url, config, reqBody, nil)
}

func (c *moonshotClientAt) CompletionsStream(ctx context.Context, config *providers.Config, reqBody []byte) (iter.Seq2[[]byte, error], error) {
	return c.chat.CompletionsStream(ctx, c.url, config, reqBody, nil)
}

func TestNewMoonshotClient(t *testing.T) {
	assert.NotNil(t, NewMoonshotClient())
}

func TestCompletions_MissingAPIKey(t *testing.T) {
	_, err := NewMoonshotClient().Completions(context.Background(), &providers.Config{}, []byte(`{}`))
	require.Error(t, err)
	assert.Contains(t, err.Error(), "API key is required")
}

func TestCompletions_NonStreamingRoundTrip(t *testing.T) {
	const wantModel = "kimi-k3"
	...
	resp, err := newMoonshotClientAt(srv.URL+"/chat/completions").Completions(
		context.Background(),
		&providers.Config{Credentials: providers.Credentials{ApiKey: "moonshot-test-key"}},
		[]byte(`{"model":"`+wantModel+`","messages":[{"role":"user","content":"hello"}]}`),
	)
	require.NoError(t, err)

	assert.Equal(t, "Bearer moonshot-test-key", gotAuth)
...
func TestCompletionsStream_RoundTrip(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		assert.Equal(t, "Bearer moonshot-key", r.Header.Get("Authorization"))
	...
	seq, err := newMoonshotClientAt(srv.URL+"/chat/completions").CompletionsStream(
		context.Background(),
		&providers.Config{Credentials: providers.Credentials{ApiKey: "moonshot-key"}},
		[]byte(`{"model":"kimi-k3","stream":true}`),
	)
	require.NoError(t, err)
...
func TestCompletions_RateLimitRetryAfter(t *testing.T) {
	...
	_, err := newMoonshotClientAt(srv.URL+"/chat/completions").Completions(
		context.Background(),
