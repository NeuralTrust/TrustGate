...
// dashScopeClientAt is a test-only wrapper that targets a custom URL so the
// OpenAI-compatible round trip can be exercised against an httptest server.
type dashScopeClientAt struct {
	chat *openai.ChatCompletionsClient
	url  string
}

func newDashScopeClientAt(url string) providers.Client {
	return &dashScopeClientAt{
		chat: openai.NewChatCompletionsClient(providers.ProviderDashScope, nil),
		url:  url,
	}
}

func (c *dashScopeClientAt) Completions(ctx context.Context, config *providers.Config, reqBody []byte) ([]byte, error) {
	return c.chat.Completions(ctx, c.url, config, reqBody, nil)
}

func (c *dashScopeClientAt) CompletionsStream(ctx context.Context, config *providers.Config, reqBody []byte) (iter.Seq2[[]byte, error], error) {
	return c.chat.CompletionsStream(ctx, c.url, config, reqBody, nil)
}

func TestNewDashScopeClient(t *testing.T) {
	assert.NotNil(t, NewDashScopeClient())
}

func TestCompletions_MissingAPIKey(t *testing.T) {
	_, err := NewDashScopeClient().Completions(context.Background(), &providers.Config{}, []byte(`{}`))
	require.Error(t, err)
...
func TestCompletions_NonStreamingRoundTrip(t *testing.T) {
	...
	resp, err := newDashScopeClientAt(srv.URL+"/chat/completions").Completions(
		context.Background(),
		&providers.Config{Credentials: providers.Credentials{ApiKey: "dashscope-test-key"}},
		[]byte(`{"model":"`+wantModel+`","messages":[{"role":"user","content":"hello"}]}`),
	)
	require.NoError(t, err)

	assert.Equal(t, "Bearer dashscope-test-key", gotAuth)
...
func TestCompletionsStream_RoundTrip(t *testing.T) {
	...
	seq, err := newDashScopeClientAt(srv.URL+"/chat/completions").CompletionsStream(
		context.Background(),
		&providers.Config{Credentials: providers.Credentials{ApiKey: "dashscope-key"}},
		[]byte(`{"model":"qwen-plus","stream":true}`),
	)
	require.NoError(t, err)
...
func TestCompletions_RateLimitRetryAfter(t *testing.T) {
	...
	_, err := newDashScopeClientAt(srv.URL+"/chat/completions").Completions(
		context.Background(),
		&providers.Config{Credentials: providers.Credentials{ApiKey: "key"}},
		[]byte(`{"model":"qwen-plus","messages":[{"role":"user","content":"hi"}]}`),
	)
	require.Error(t, err)
