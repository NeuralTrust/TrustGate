...
//
// If the two formats are wire-compatible the body is returned unmodified,
// except when provider-specific extensions require canonical filtering.
func (r *Registry) AdaptResponse(body []byte, source, target Format) ([]byte, error) {
	...
	dropProviderExtensionsForCrossFormat(source, target, canonical)
...
//
// If the two formats are wire-compatible the chunk is returned unmodified,
// except when provider-specific extensions require canonical filtering.
//
...
func dropProviderExtensionsForCrossFormat(source, target Format, resp *CanonicalResponse) {
	...
	if source != target {
		resp.ProviderExtensions = nil
	}
}

func dropStreamProviderExtensionsForCrossFormat(source, target Format, chunk *CanonicalStreamChunk) {
	if chunk == nil {
		return
	}
	if source != target {
