...
// NormalizeBedrockRequestForModel rewrites a request body built for another
// Bedrock family into the schema model expects. It is the last seam before
// InvokeModel, where the model is finally known, and it covers the bodies no
// earlier conversion can reach: a client calling the Bedrock-native route with
// the Claude-on-Bedrock shape it used before the registry moved to Nova.
//
// Only Amazon Nova is rewritten. Every other family either tolerates the extra
// keys or fails on something this cannot repair, and a needless round trip
// through the canonical model would drop whatever it cannot represent.
