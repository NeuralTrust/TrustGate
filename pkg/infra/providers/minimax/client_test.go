...
// miniMaxClientAt is a test-only wrapper that targets a custom URL so the
// OpenAI-compatible round trip can be exercised against an httptest server.
type miniMaxClientAt struct {
	chat *openai.ChatCompletionsClient
	url  string
}

func newMiniMaxClientAt(url string) providers.Client {
	return &miniMaxClientAt{
		chat: openai.NewChatCompletionsClient(providers.ProviderMiniMax, nil),
		url:  url,
	}
}

func (c *miniMaxClientAt) Completions(ctx context.Context, config *providers.Config, reqBody []byte) ([]byte, error) {
	return c.chat.Completions(ctx, c.url, config, reqBody, nil)
}

func (c *miniMaxClientAt) CompletionsStream(ctx context.Context, config *providers.Config, reqBody []byte) (iter.Seq2[[]byte, error], error) {
	return c.chat.CompletionsStream(ctx, c.url, config, reqBody, nil)
}

func TestNewMiniMaxClient(t *testing.T) {
	assert.NotNil(t, NewMiniMaxClient())
}

func TestCompletions_MissingAPIKey(t *testing.T) {
	_, err := NewMiniMaxClient().Completions(context.Background(), &providers.Config{}, []byte(`{}`))
	require.Error(t, err)
	assert.Contains(t, err.Error(), "API key is required")
}

func TestCompletions_NonStreamingRoundTrip(t *testing.T) {
	const wantModel = "MiniMax-M2"
	...
	resp, err := newMiniMaxClientAt(srv.URL+"/chat/completions").Completions(
		context.Background(),
		&providers.Config{Credentials: providers.Credentials{ApiKey: "minimax-test-key"}},
		[]byte(`{"model":"`+wantModel+`","messages":[{"role":"user","content":"hello"}]}`),
	)
	require.NoError(t, err)

	assert.Equal(t, "Bearer minimax-test-key", gotAuth)
...
func TestCompletionsStream_RoundTrip(t *testing.T) {
	...
	seq, err := newMiniMaxClientAt(srv.URL+"/chat/completions").CompletionsStream(
		context.Background(),
		&providers.Config{Credentials: providers.Credentials{ApiKey: "minimax-key"}},
		[]byte(`{"model":"MiniMax-M2","stream":true}`),
	)
	require.NoError(t, err)
...
func TestCompletions_RateLimitRetryAfter(t *testing.T) {
	...
	_, err := newMiniMaxClientAt(srv.URL+"/chat/completions").Completions(
		context.Background(),
		&providers.Config{Credentials: providers.Credentials{ApiKey: "key"}},
		[]byte(`{"model":"MiniMax-M2","messages":[{"role":"user","content":"hi"}]}`),
	)
	require.Error(t, err)
