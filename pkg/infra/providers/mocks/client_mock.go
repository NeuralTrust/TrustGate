...
// CompletionsStream provides a mock function with given fields: req, config, reqBody, streamChan, breakChan
func (_m *Client) CompletionsStream(req *types.RequestContext, config *providers.Config, reqBody []byte, streamChan chan []byte, breakChan chan struct{}) error {
	ret := _m.Called(req, config, reqBody, streamChan, breakChan)
	...
	var r0 error
	if rf, ok := ret.Get(0).(func(*types.RequestContext, *providers.Config, []byte, chan []byte, chan struct{}) error); ok {
		r0 = rf(req, config, reqBody, streamChan, breakChan)
	} else {
		r0 = ret.Error(0)
	}
...
func (_e *Client_Expecter) CompletionsStream(req interface{}, config interface{}, reqBody interface{}, streamChan interface{}, breakChan interface{}) *Client_CompletionsStream_Call {
	return &Client_CompletionsStream_Call{Call: _e.mock.On("CompletionsStream", req, config, reqBody, streamChan, breakChan)}
}

func (_c *Client_CompletionsStream_Call) Run(run func(req *types.RequestContext, config *providers.Config, reqBody []byte, streamChan chan []byte, breakChan chan struct{})) *Client_CompletionsStream_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(*types.RequestContext), args[1].(*providers.Config), args[2].([]byte), args[3].(chan []byte), args[4].(chan struct{}))
	})
	return _c
}
...
func (_c *Client_CompletionsStream_Call) RunAndReturn(run func(*types.RequestContext, *providers.Config, []byte, chan []byte, chan struct{}) error) *Client_CompletionsStream_Call {
	_c.Call.Return(run)
	return _c
}
