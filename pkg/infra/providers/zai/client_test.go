...
// zAIClientAt is a test-only wrapper that targets a custom URL so the
// OpenAI-compatible round trip can be exercised against an httptest server.
type zAIClientAt struct {
	chat *openai.ChatCompletionsClient
	url  string
}

func newZAIClientAt(url string) providers.Client {
	return &zAIClientAt{
...
func (c *zAIClientAt) Completions(ctx context.Context, config *providers.Config, reqBody []byte) ([]byte, error) {
	return c.chat.Completions(ctx, c.url, config, reqBody, nil)
}

func (c *zAIClientAt) CompletionsStream(ctx context.Context, config *providers.Config, reqBody []byte) (iter.Seq2[[]byte, error], error) {
	return c.chat.CompletionsStream(ctx, c.url, config, reqBody, nil)
}

func TestNewZAIClient(t *testing.T) {
	assert.NotNil(t, NewZAIClient())
}

func TestCompletions_MissingAPIKey(t *testing.T) {
	_, err := NewZAIClient().Completions(context.Background(), &providers.Config{}, []byte(`{}`))
	require.Error(t, err)
	assert.Contains(t, err.Error(), "API key is required")
}

func TestCompletions_NonStreamingRoundTrip(t *testing.T) {
	const wantModel = "glm-4.6"
	...
	resp, err := newZAIClientAt(srv.URL+"/chat/completions").Completions(
		context.Background(),
		&providers.Config{Credentials: providers.Credentials{ApiKey: "zai-test-key"}},
		[]byte(`{"model":"`+wantModel+`","messages":[{"role":"user","content":"hello"}]}`),
	)
	require.NoError(t, err)

	assert.Equal(t, "Bearer zai-test-key", gotAuth)
...
func TestCompletionsStream_RoundTrip(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		assert.Equal(t, "Bearer zai-key", r.Header.Get("Authorization"))
	...
	seq, err := newZAIClientAt(srv.URL+"/chat/completions").CompletionsStream(
		context.Background(),
		&providers.Config{Credentials: providers.Credentials{ApiKey: "zai-key"}},
		[]byte(`{"model":"glm-4.6","stream":true}`),
	)
	require.NoError(t, err)
...
func TestCompletions_RateLimitRetryAfter(t *testing.T) {
	...
	_, err := newZAIClientAt(srv.URL+"/chat/completions").Completions(
		context.Background(),
		&providers.Config{Credentials: providers.Credentials{ApiKey: "key"}},
		[]byte(`{"model":"glm-4.6","messages":[{"role":"user","content":"hi"}]}`),
	)
	require.Error(t, err)
