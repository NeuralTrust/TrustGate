...
func TestPriceFor_CustomOverlay(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			p := &Plugin{}
			in, out, found := p.priceFor(context.Background(), &config{CustomPricing: tt.pricing}, "openai", tt.model)
...
func TestPriceFor_BuiltinFallback(t *testing.T) {
	...
	p := &Plugin{pricing: resolver}
	in, out, found := p.priceFor(context.Background(), &config{}, "openai", "gpt-4o-mini-2024-07-18")
...
func TestPriceFor_CustomWinsOverBuiltin(t *testing.T) {
	...
	p := &Plugin{pricing: resolver}
	cfg := &config{CustomPricing: map[string]customPrice{"gpt-4o-mini": {Input: 0.1, Output: 0.2}}}
	in, out, found := p.priceFor(context.Background(), cfg, "openai", "gpt-4o-mini")
	require.True(t, found)
...
func TestPriceFor_NoResolverNoCustom(t *testing.T) {
	p := &Plugin{}
	_, _, found := p.priceFor(context.Background(), &config{}, "openai", "gpt-4o-mini")
