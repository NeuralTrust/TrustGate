...
func (r *UpstreamRepository) GetUpstream(ctx context.Context, id uuid.UUID) (*upstream.Upstream, error) {
	...
	if err := dbFromContext(ctx, r.db).WithContext(ctx).First(&entity, "id = ?", id).Error; err != nil {
		return nil, err
	}
	return &entity, nil
}

func (r *UpstreamRepository) CreateUpstream(ctx context.Context, u *upstream.Upstream) error {
	return dbFromContext(ctx, r.db).WithContext(ctx).Create(u).Error
}
...
func (r *UpstreamRepository) ListUpstreams(ctx context.Context, gatewayID uuid.UUID, offset, limit int) ([]upstream.Upstream, error) {
	...
	query := dbFromContext(ctx, r.db).WithContext(ctx).Where("gateway_id = ?", gatewayID)
...
func (r *UpstreamRepository) UpdateUpstream(ctx context.Context, upstream *upstream.Upstream) error {
	return dbFromContext(ctx, r.db).WithContext(ctx).Save(upstream).Error
}
...
func (r *UpstreamRepository) DeleteUpstream(ctx context.Context, id string) error {
	...
	return db.Delete(&upstream.Upstream{}, "id = ?", id).Error
}
