...
func TestRepository_MCPTargetProtocolModeRoundTrip(t *testing.T) {
	t.Parallel()
	tests := []struct {
		name   string
		...
		{
			name:   "old payload defaults to auto",
			stored: []byte(`{"url":"https://mcp.example.com/mcp"}`),
			want:   domain.MCPProtocolModeAuto,
		},
		{
			name:   "auto",
			target: &domain.MCPTarget{URL: "https://mcp.example.com/mcp", ProtocolMode: domain.MCPProtocolModeAuto},
			want:   domain.MCPProtocolModeAuto,
		},
		{
			name:   "modern",
			target: &domain.MCPTarget{URL: "https://mcp.example.com/mcp", ProtocolMode: domain.MCPProtocolModeModern},
			want:   domain.MCPProtocolModeModern,
		},
		{
			name:   "legacy",
			target: &domain.MCPTarget{URL: "https://mcp.example.com/mcp", ProtocolMode: domain.MCPProtocolModeLegacy},
			want:   domain.MCPProtocolModeLegacy,
		},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.target != nil {
				var err error
				stored, err = marshalMCPTarget(tc.target)
				if err != nil {
					t.Fatalf("marshalMCPTarget: %v", err)
				}
			}
			now := time.Now().UTC()
			reg, err := (&Repository{}).scanRegistry(fakeRow{values: []any{
				ids.New[ids.RegistryKind](),
				ids.New[ids.GatewayKind](),
				"mcp",
				string(domain.TypeMCP),
				true,
				(*string)(nil),
				[]byte(nil),
				[]byte(nil),
			...
			if reg.MCPTarget == nil {
				t.Fatal("MCPTarget is nil")
			}
			if reg.MCPTarget.ProtocolMode != tc.want {
				t.Fatalf("ProtocolMode = %q, want %q", reg.MCPTarget.ProtocolMode, tc.want)
			}
		})
	}
}
