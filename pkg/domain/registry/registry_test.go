...
func TestTargetAuth_Validate(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "aws ok with access keys", auth: &TargetAuth{Type: AuthTypeAWS, AWS: &AWSAuth{Region: "us-east-1", AccessKeyID: "AKIA", SecretAccessKey: "secret"}}},
		{name: "aws ok assume role", auth: &TargetAuth{Type: AuthTypeAWS, AWS: &AWSAuth{Region: "us-east-1", UseRole: true, Role: "arn:aws:iam::123:role/x"}}},
		{name: "aws missing region", auth: &TargetAuth{Type: AuthTypeAWS, AWS: &AWSAuth{AccessKeyID: "AKIA", SecretAccessKey: "secret"}}, wantErr: true},
		{name: "aws missing keys", auth: &TargetAuth{Type: AuthTypeAWS, AWS: &AWSAuth{Region: "us-east-1"}}, wantErr: true},
		{name: "aws assume role missing role", auth: &TargetAuth{Type: AuthTypeAWS, AWS: &AWSAuth{Region: "us-east-1", UseRole: true}}, wantErr: true},
