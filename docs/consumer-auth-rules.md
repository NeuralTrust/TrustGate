...
|---|---|---|---|
| LLM / A2A | inline | varios | `api_key`, `oauth2`, `oidc`, `mtls` |
| MCP | inline | varios | `api_key`, `oauth2`, `mtls` (❌ `oidc`) |
| LLM / A2A | role_based | máx. 1 | `oauth2` o `oidc` |
...
- `role_based` ⇒ máx. 1 auth y de tipo identity-provider (`oauth2`/`oidc`).
- `MCP` ⇒ nunca `oidc`.
