...
As many as an operator asks for.
...
accounts of the same SaaS. A server that is one URL behind per-user OAuth used
to have no such configuration — the gateway registers its client itself
(`registration: auto`) or the platform holds one, and each user signs in with
their own account — so the catalog declared `multi_instance: false` for it and
the creator refused a second registry (`ErrSingleInstanceServer`).

**Whose account it uses is now such a configuration** (§12.3): one instance
where each user connects their own, another on the account the team shares, a
third behind an API key. Half the catalog was declared single-instance on the
premise that there was nothing to differ in, and that premise is gone — so the
flag, the loader's requirement, the creator's refusal and the console's
single-instance callout are gone with it.

What the refusal was protecting against is still real and is now the operator's
to weigh: a second instance means one to pick on every install and uninstall,
every one of its tools renamed with an instance prefix (`resolveNames`'
`perInstance`), and two Access rows to grant. Someone who creates a copy of an
instance for no reason gets exactly that, and can delete it.

`self_service` is unaffected: it answers a different question (can a user
install this with nothing configured by an admin), it is still declared per
entry, and the loader still refuses an entry that forgets it.
